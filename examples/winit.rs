...
fn main() {
	...
	event_loop.run(move |event, event_loop| {
		...
		// This can be removed to allow ControlFlow::Poll to poll on each cpu
		// cycle Alternatively, you can set ControlFlow::Wait or use
		...
		if let winit::event::Event::NewEvents(winit::event::StartCause::Init) = event {
			...
			// We have to request a redraw here to have the icon actually show
			// up. Winit only exposes a redraw method on the Window so we use
