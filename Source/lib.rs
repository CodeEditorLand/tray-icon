...
//!   Windows, a win32 event loop and on Linux, a gtk event loop. It doesn't
//!   need to be the main thread but you have to create the tray icon on the
...
//!
//! On Linux, `gtk`, `libxdo` is used to make the predfined `Copy`, `Cut`,
//! `Paste` and `SelectAll` menu items work and `libappindicator` or
...
//! happens on the tray icon ```no_run
...
impl TrayIconBuilder {
	...
	/// ## Platform-specific:
	///
	/// - **Linux**: once a menu is set, it cannot be removed or replaced but
...
///
/// This type is reference-counted and the icon is removed when the last
/// instance is dropped.
#[derive(Clone)]
pub struct TrayIcon {
	id:TrayIconId,
...
impl TrayIcon {
	...
	/// Builds and adds a new tray icon to the system tray with the specified
	/// Id.
	///
	/// See [`TrayIcon::new`] for more info.
	...
	/// ## Platform-specific:
	///
	/// - **Linux**: once a menu is set it cannot be removed so `None` has no
	///   effect
	...
	pub fn set_icon_with_as_template(&self, icon:Option<Icon>, is_template:bool) -> Result<()> {
		#[cfg(target_os = "macos")]
		return self.tray.borrow_mut().set_icon_with_as_template(icon, is_template);
		#[cfg(not(target_os = "macos"))]
		{
			let _ = icon;
	...
	///
	/// - **Linux**: Unsupported.
	pub fn rect(&self) -> Option<Rect> { self.tray.borrow().rect() }
}
...
/// ## Platform-specific:
///
/// - **Linux**: Unsupported. The event is not emmited even though the icon is
...
impl TrayIconEvent {
	...
	/// has been called with a `Some` value.
	pub fn receiver<'a>() -> &'a TrayIconEventReceiver { &TRAY_CHANNEL.1 }
