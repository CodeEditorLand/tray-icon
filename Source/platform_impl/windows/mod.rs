...
impl TrayIcon {
	pub fn new(id:TrayIconId, attrs:TrayIconAttributes) -> crate::Result<Self> {
		...
		unsafe {
			...
			Ok(Self { hwnd, internal_id, menu:attrs.menu })
		}
	}
	...
	pub fn set_show_menu_on_left_click(&mut self, enable:bool) {
		unsafe {
			SendMessageW(self.hwnd, WM_USER_SHOW_MENU_ON_LEFT_CLICK, enable as usize, 0);
		}
	}
...
fn get_tray_rect(id:u32, hwnd:HWND) -> Option<RECT> {
	...
	let mut rect = RECT { left:0, bottom:0, right:0, top:0 };
